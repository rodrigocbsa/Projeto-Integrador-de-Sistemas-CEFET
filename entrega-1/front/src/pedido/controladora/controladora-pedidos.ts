...
export class ControladoraPedidos{
    ...
    async listarMesasDasReservasDeAgora(){
        try{
            ...
            if(reservasDeAgora.length == 0){
                this.visao.exibirNotificacao(["Não há reservas em andamento."],"info");
                return;
            }
    ...
    adicionarItemAoPedidoDaMesa(idMesa: string){
        ...
        try{
            ...
            this.visao.exibirNotificacao(["Item adicionado com sucesso"],'success');
    ...
    async adicionarPedidoViaDashboardNaMesa(id_reserva: string){
        try{
            ...
            window.location.href = 'http://localhost:5173/front/pages/pedidos.html';
        } catch(error: any){
