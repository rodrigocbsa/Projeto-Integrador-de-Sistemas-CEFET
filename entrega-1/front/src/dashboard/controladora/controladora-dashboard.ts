...
export class ControladoraDashboard {
  ...
  async gerarRelatorio() {
    try {
      ...
      if(array.length == 0){
        this.visao.exibirNotificacao(["Não há reservas para o período informado."],'info');
        return;
      }
  ...
  async listarReservas() {
    try {
      ...
      if(reservasFiltradas.length == 0){
        this.visao.exibirNotificacao(["Não há mais reservas para hoje."],'info');
  ...
  async cancelarReserva(id_reserva: string){
    try {
      ...
      if(reserva.id != Number(id_reserva)){
        this.visao.exibirNotificacao(["Reserva não encontrada."],'warning');
        return;
      }
    ...
    if(itens.length != 0){
      ...
      try{
          ...
          if ( pedidoCadastrado.success && reservaConcluida.success ) {
            this.visao.exibirNotificacao(["Conta fechada com sucesso!"],'success');
            ...
            this.visao.exibirNotificacao(["Erro ao fechar a conta!"],'error');
          }
      } catch(error: any){
        this.visao.exibirNotificacao([error.message],"error");
      }
    } else {
      this.visao.exibirNotificacao(["Não há pedidos nesta reserva."],'warning'); // Não haver pedidos na mesa, no meu restaurante, é o equivalente a ter que cancelar uma reserva.
    }
  }
