...
$app->get( '/funcionario/gerente/relatorios/:id', $middlewareIsAdmin, function( $req, $res ) use ( $pdo ) 
{
    $content = [];
...
$app->post('/login', function( $req, $res ) use ($pdo) {
    ...
    if ( $resposta === false ) {
        ...
        $res->json(['success' => true, 'nome' => $resposta['nome'], 'usuario' => $resposta['usuario'], 'funcionario' => $resposta['funcionario'] ]);
    }

});

$app->get('/funcionario/autenticado', $middlewareIsLogged, function($req,$res) use ($pdo) {
    $res->status( 200 )->send( 'Acesso autenticado.' );
});

$app->get('/funcionario/permitido', $middlewareIsAdmin, function($req,$res) use ($pdo) {
...
$app->post( '/funcionario/cadastro',$middlewareIsAdmin, function( $req, $res ) use ( $pdo ) 
{
    $dados = (array) $req->body();
    ...
    $acesso = htmlspecialchars($dados['acesso'] ?? '');


    $controladora = criarControladoraFuncionario($pdo);

    $success = $controladora->postFuncionario($usuario,$senha,$nome,$acesso);
    if ( $success ) {
