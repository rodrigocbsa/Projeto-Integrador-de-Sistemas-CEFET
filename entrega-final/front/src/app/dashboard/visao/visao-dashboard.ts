...
export class VisaoDashboard {
  ...
  iniciar(): void {
    ...
    const botaoReservas = document.querySelector(sels.reservas) as HTMLButtonElement;
    botaoReservas.onclick = (event: Event) => {
      event.preventDefault();
      this.controladora?.listarReservas();
    };

    const botaoRelatorio = document.querySelector(sels.relatorio) as HTMLButtonElement;
    botaoRelatorio.onclick = (event: Event) => {
      event.preventDefault();
      this.controladora?.gerarRelatorio();
    };

    const botaoFecharOffCanvas = document.querySelector(sels.fecharOffCanvas) as HTMLButtonElement;
  ...
  desenharExtras(totalReservasFeitas: string, totalPedidosRealizados: string, receitaTotal: string) {
    (document.querySelector(sels.reservasFeitas) as HTMLTitleElement).innerHTML = totalReservasFeitas;
    (document.querySelector(sels.pedidosRealizados) as HTMLTitleElement).innerHTML = totalPedidosRealizados;
    (document.querySelector(sels.receitaTotal) as HTMLTitleElement).innerHTML = `R$ ${receitaTotal},00`;
  }
  ...
  desenharGrafico(datasOrdenadas: Array<string>, totaisPorData: Array<number>) {
    const canva = <HTMLCanvasElement> document.querySelector(sels.grafico);
  ...
  valorDataInicioRelatorio(): string {
    const el = <HTMLInputElement> document.querySelector(sels.dti);
    return el.value;
  }
  valorDataFinalRelatorio(): string {
    const el = <HTMLInputElement> document.querySelector(sels.dtf);
    return el.value;
  }
  ...
  desenharPedidosNoOverlay(itens: Item[],idMesa: number,idReserva: number) {
    ...
    const textArea = document.querySelector(sels.resumo) as HTMLTextAreaElement;
    itens.forEach(item => {
    ...
    const totalEl = document.querySelector(sels.total) as HTMLParagraphElement;
    totalEl.innerText = `R$ ${totalSemDesconto},00`;

    const totalEl2 = document.querySelector(sels.totalFinal) as HTMLParagraphElement;
  ...
  valorModoPagamento(): string{
    const el = <HTMLInputElement> document.querySelector(sels.pagamento);
    return el.value;
  }
  ...
  limparOffCanvas(){
    const textArea = document.querySelector(sels.resumo) as HTMLTextAreaElement;
    textArea.value = '';
    const total = document.querySelector(sels.total) as HTMLParagraphElement;
    total.innerText = '';
    const totalFinal = document.querySelector(sels.totalFinal) as HTMLParagraphElement;
    totalFinal.innerText = '';
  }

  fecharOffCanvas(){
    const btn = document.querySelector(sels.fecharOffCanvas) as HTMLButtonElement;
    btn.click();
  }
}