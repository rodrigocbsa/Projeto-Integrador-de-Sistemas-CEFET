...
export class ControladoraRelatoriosGerente {
    ...
    async gerarRelatorioVendasPagamento() {
        try {
            ...
            if (array.length == 0) {
                this.visao.exibirNotificacao(["Não há dados para o período informado."], TIPOS_NOTIFICACAO.INFO);
                return;
            }
    ...
    async gerarRelatorioVendasFuncionario() {
        try{
            ...
            if(array.length == 0){
                this.visao.exibirNotificacao(["Não há dados para a consulta no período informado."],TIPOS_NOTIFICACAO.INFO);
                return;
            }
